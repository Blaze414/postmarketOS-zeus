...
static inline struct sim_dsc_375 *to_sim_dsc_375(struct drm_panel *panel)
{
	return container_of_const(panel, struct sim_dsc_375, panel);
}

static void sim_dsc_375_reset(struct sim_dsc_375 *ctx)
{
...
static int sim_dsc_375_on(struct sim_dsc_375 *ctx)
{
...
static int sim_dsc_375_off(struct sim_dsc_375 *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int sim_dsc_375_prepare(struct drm_panel *panel)
{
	struct sim_dsc_375 *ctx = to_sim_dsc_375(panel);
	struct device *dev = &ctx->dsi->dev;
	struct drm_dsc_picture_parameter_set pps;
	int ret;

	sim_dsc_375_reset(ctx);

	ret = sim_dsc_375_on(ctx);
	if (ret < 0) {
...
static int sim_dsc_375_unprepare(struct drm_panel *panel)
{
	struct sim_dsc_375 *ctx = to_sim_dsc_375(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = sim_dsc_375_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode sim_dsc_375_mode = {
...
static int sim_dsc_375_get_modes(struct drm_panel *panel,
				 struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &sim_dsc_375_mode);
}

static const struct drm_panel_funcs sim_dsc_375_panel_funcs = {
	.prepare = sim_dsc_375_prepare,
	.unprepare = sim_dsc_375_unprepare,
	.get_modes = sim_dsc_375_get_modes,
};

static int sim_dsc_375_bl_update_status(struct backlight_device *bl)
{
...
static int sim_dsc_375_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops sim_dsc_375_bl_ops = {
	.update_status = sim_dsc_375_bl_update_status,
	.get_brightness = sim_dsc_375_bl_get_brightness,
};

static struct backlight_device *
sim_dsc_375_create_backlight(struct mipi_dsi_device *dsi)
{
	...
	return devm_backlight_device_register(dev, dev_name(dev), dev, dsi,
					      &sim_dsc_375_bl_ops, &props);
}

static int sim_dsc_375_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct sim_dsc_375 *ctx;
	int ret;

	ctx = devm_drm_panel_alloc(dev, struct sim_dsc_375, panel,
				   &sim_dsc_375_panel_funcs,
	...
	ctx->panel.backlight = sim_dsc_375_create_backlight(dsi);
...
static void sim_dsc_375_remove(struct mipi_dsi_device *dsi)
{
	struct sim_dsc_375 *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id sim_dsc_375_of_match[] = {
	{ .compatible = "sim,dsc-375" }, // FIXME
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, sim_dsc_375_of_match);

static struct mipi_dsi_driver sim_dsc_375_driver = {
	.probe = sim_dsc_375_probe,
	.remove = sim_dsc_375_remove,
	.driver = {
		.name = "panel-sim-dsc-375",
		.of_match_table = sim_dsc_375_of_match,
	},
};
module_mipi_dsi_driver(sim_dsc_375_driver);

MODULE_AUTHOR("linux-mdss-dsi-panel-driver-generator <fix@me>"); // FIXME
MODULE_DESCRIPTION("DRM driver for Simulator cmd mode DSC 3.75:1 dsi panel");
MODULE_LICENSE("GPL");
