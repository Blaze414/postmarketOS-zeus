...
static inline
struct nt35695b_truly_sl *to_nt35695b_truly_sl(struct drm_panel *panel)
{
	return container_of_const(panel, struct nt35695b_truly_sl, panel);
}

static void nt35695b_truly_sl_reset(struct nt35695b_truly_sl *ctx)
{
...
static int nt35695b_truly_sl_on(struct nt35695b_truly_sl *ctx)
{
...
static int nt35695b_truly_sl_off(struct nt35695b_truly_sl *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int nt35695b_truly_sl_prepare(struct drm_panel *panel)
{
	struct nt35695b_truly_sl *ctx = to_nt35695b_truly_sl(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	nt35695b_truly_sl_reset(ctx);

	ret = nt35695b_truly_sl_on(ctx);
	if (ret < 0) {
...
static int nt35695b_truly_sl_unprepare(struct drm_panel *panel)
{
	struct nt35695b_truly_sl *ctx = to_nt35695b_truly_sl(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = nt35695b_truly_sl_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode nt35695b_truly_sl_mode = {
...
static int nt35695b_truly_sl_get_modes(struct drm_panel *panel,
				       struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &nt35695b_truly_sl_mode);
}

static const struct drm_panel_funcs nt35695b_truly_sl_panel_funcs = {
	.prepare = nt35695b_truly_sl_prepare,
	.unprepare = nt35695b_truly_sl_unprepare,
	.get_modes = nt35695b_truly_sl_get_modes,
};

static int nt35695b_truly_sl_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct nt35695b_truly_sl *ctx;
	int ret;

	ctx = devm_drm_panel_alloc(dev, struct nt35695b_truly_sl, panel,
				   &nt35695b_truly_sl_panel_funcs,
...
static void nt35695b_truly_sl_remove(struct mipi_dsi_device *dsi)
{
	struct nt35695b_truly_sl *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id nt35695b_truly_sl_of_match[] = {
	{ .compatible = "mdss,nt35695b-truly-sl" }, // FIXME
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, nt35695b_truly_sl_of_match);

static struct mipi_dsi_driver nt35695b_truly_sl_driver = {
	.probe = nt35695b_truly_sl_probe,
	.remove = nt35695b_truly_sl_remove,
	.driver = {
		.name = "panel-nt35695b-truly-sl",
		.of_match_table = nt35695b_truly_sl_of_match,
	},
};
module_mipi_dsi_driver(nt35695b_truly_sl_driver);

MODULE_AUTHOR("linux-mdss-dsi-panel-driver-generator <fix@me>"); // FIXME
MODULE_DESCRIPTION("DRM driver for nt35695b truly fhd command mode split link dsi panel");
MODULE_LICENSE("GPL");
